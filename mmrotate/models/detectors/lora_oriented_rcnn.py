...
class LoRA_OrientedRCNN(RotatedTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(LoRA_OrientedRCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def _freeze_stages(self):
        ...
        #     if 'lora_' not in n:
        #     #if 'lateral_convs' not in n and '0' not in n and '3' not in n:
        #         p.requires_grad = False

        for n, p in  self.roi_head.named_parameters():
            #print(n,p.requires_grad)
            if 'lora_' not in n and 'fc_cls' not in n and 'fc_reg' not in n:
                p.requires_grad = False
            #print(n,p.requires_grad)
  
        # for n, p in  self.rpn_head.named_parameters():
        #     if 'lora_' not in n:
        #         p.requires_grad = False
